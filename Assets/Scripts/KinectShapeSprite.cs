...
public class KinectShapeSprite : MonoBehaviour {
	...
	void Start () 
	{
		...
		for(int i = 0; i < numColliders; i++)
		{
			...
			if(sColObjectName.Equals("HipCenterCollider"))
				sprite.reference = jointColliders[i].transform;
	...
	void Update () 
	{
		...
		if(manager.IsUserDetected())
		{
			...
			for(int i = 0; i < numColliders; i++)
			{
				if(manager.IsJointTracked(userId, i))
				{
					...
					if(posJoint != Vector3.zero)
					{
						...
						float zDistance = posJoint.z - Camera.main.transform.position.z + zOffset;
			...
			print(foregroundRect);
			
			sprite.sprite = Sprite.Create(foregroundTex, new Rect(0,0,foregroundTex.width,foregroundTex.height),new Vector2(0.5f,0),50);
		}
